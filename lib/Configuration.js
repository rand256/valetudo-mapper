...
const Configuration = function(options) {
    if (options.customConfigPath) {
        this.location = path.resolve(options.customConfigPath);
    } else {
    ...
    } else if (options.customConfigPath && !options.customConfigCreate) {
        console.log("Configuration file doesn't exist:", this.location);
        console.log("Please use \"-c\" flag to create it or check the path is valid");
        process.exit(1);
    } else {
